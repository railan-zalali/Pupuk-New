                        ...
                        <div class="mt-4">
                            ...
                            <div class="mt-2 flex items-center text-xs text-blue-100">
                                ...
                                @else
                                    ...
                                    <span>{{ number_format(abs($data['salesChangeToday']), 2) }}% dari kemarin</span>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <div class="mt-4">
                            <div class="relative h-1 w-full bg-green-400/30 rounded-full overflow-hidden">
                                <div class="absolute h-full bg-white rounded-full"
                                    style="width: {{ min(max($data['salesChangeThisMonth'], 0), 100) }}%"></div>
                            </div>
                            <div class="mt-2 flex items-center text-xs text-green-100">
                                @if ($data['salesChangeThisMonth'] >= 0)
                                    <svg class="w-3 h-3 mr-1" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd"
                                            d="M12 7a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0V8.414l-4.293 4.293a1 1 0 01-1.414 0L8 10.414l-4.293 4.293a1 1 0 01-1.414-1.414l5-5a1 1 0 011.414 0L11 10.586 14.586 7H12z"
                                            clip-rule="evenodd" />
                                    </svg>
                                    <span>{{ number_format($data['salesChangeThisMonth'], 2) }}% dari bulan lalu</span>
                                @else
                                    <svg class="w-3 h-3 mr-1" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd"
                                            d="M12 13a1 1 0 110 2H7a1 1 0 01-1-1v-5a1 1 0 112 0v2.586l4.293-4.293a1 1 0 011.414 0L16 9.586l4.293-4.293a1 1 0 111.414 1.414L16.414 12l4.293 4.293a1 1 0 01-1.414 1.414L15 13.414l-2.293 2.293a1 1 0 01-1.414 0L8 12.414 5.414 15H8z"
                                    ...
                                    <span>{{ number_format(abs($data['salesChangeThisMonth']), 2) }}% dari bulan
                                        lalu</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="mt-4 flex items-center gap-2">
                            @if ($data['lowStockProducts'] > 0)
                                <div
                                    ...
                                    <span>{{ $data['lowStockProducts'] }} stok menipis</span>
                                </div>
                            @else
                                <div
            ...
            <div
                ...
                <div class="px-6 py-5 relative">
                    ...
                    <div class="relative">
                        ...
                        <div class="mt-4 flex items-center gap-2">
                            @if ($data['totalUpcomingCredits'] > 0)
                                <div
                                    ...
                                    <span>{{ $data['totalUpcomingCredits'] }} akan jatuh tempo</span>
                                </div>
                            @else
                                <div
                                    class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        ...
                        <div class="min-w-full rounded-lg border border-gray-200 dark:border-gray-700">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                                    @forelse($data['lowStockAlerts'] as $product)
                ...
                <div
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 flex items-center">
                            ...
                            Draft Transaksi Menunggu
                        </h3>
                        @if ($data['expiringDrafts'] > 0)
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-200">
                                {{ $data['expiringDrafts'] }} segera expire
                            </span>
                        @endif
                    </div>

                    <div class="space-y-3">
                        @foreach ($data['draftSales'] as $draft)
                            @php
                                $daysOld = \Carbon\Carbon::parse($draft->created_at)->diffInDays(now());
                                $isExpiringSoon = 30 - $daysOld <= 3;
                            @endphp
                            <div
                                class="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors {{ $isExpiringSoon ? 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800' : 'bg-gray-50 dark:bg-gray-700/50' }}">
                                <div class="flex-1">
                                    <div class="flex items-center">
                                    ...
                                    <div
                                        ...
                                        <span>Rp {{ number_format($draft->total_amount, 0, ',', '.') }}</span>
                                        <span>•</span>
                                        <span
                                            class="{{ $isExpiringSoon ? 'text-red-600 dark:text-red-400 font-medium' : '' }}">
                                            {{ $daysOld == 0 ? 'Hari ini' : ($daysOld == 1 ? 'Kemarin' : $daysOld . ' hari lalu') }}
                                        </span>
                                    </div>
                                </div>
                ...
                <div class="mt-2 overflow-x-auto">
                    <div class="min-w-full rounded-lg border border-gray-200 dark:border-gray-700">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                                @forelse($data['expiringProducts'] as $product)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                        <td
                                            class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                            {{ $product->code }}
                                    ...
                                    <div class="flex">
                                        ...
                                        <div class="ml-3">
                                            ...
                                            <div class="mt-2 text-sm text-red-700 dark:text-red-200">
                                                <p>Terdapat {{ $data['lowStockProducts'] }} produk dengan stok rendah.
                                                    Segera
                                    ...
                                    <div class="flex">
                                        ...
                                        <div class="ml-3">
                                            ...
                                            <div class="mt-2 text-sm text-orange-700 dark:text-orange-200">
                                                <p>Terdapat {{ $data['totalUpcomingCredits'] }} transaksi kredit yang
                                                    akan
            ...
            function initDailySalesChart() {
                const dailySalesChart = new Chart(
                    document.getElementById('dailySalesChart'), {
                        type: 'line',
                        data: {
                            labels: @json($data['dailySales']->pluck('date')->map(fn($date) => \Carbon\Carbon::parse($date)->format('d/m'))),
                            datasets: [{
                                label: 'Penjualan Harian',
                                data: @json($data['dailySales']->pluck('total')),
            ...
            function initTopProductsChart() {
                const topProductsChart = new Chart(
                    document.getElementById('topProductsChart'), {
                        type: 'bar',
                        data: {
                            labels: @json($data['topProducts']->pluck('name')),
                            datasets: [{
                                label: 'Unit Terjual',
                                data: @json($data['topProducts']->pluck('total_sold')),
                                backgroundColor: [
                                    'rgba(59, 130, 246, 0.8)',
                                    'rgba(16, 185, 129, 0.8)',
                                    'rgba(245, 158, 11, 0.8)',
            ...
            function initSalesTrendChart() {
                const salesTrendChart = new Chart(
                    document.getElementById('salesTrendChart'), {
                        type: 'line',
                        data: {
                            labels: @json($data['salesTrend']->pluck('date')->map(fn($date) => \Carbon\Carbon::parse($date)->format('M y'))),
                            datasets: [{
                                label: 'Tren Penjualan',
                                data: @json($data['salesTrend']->pluck('total')),
