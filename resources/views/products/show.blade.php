        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in-up delay-100">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="card p-6 flex flex-col items-center text-center">
                    <div
                        class="w-48 h-48 rounded-2xl bg-gray-50 dark:bg-gray-700/50 flex items-center justify-center overflow-hidden mb-4 border border-gray-100 dark:border-gray-700">
                        @if ($product->image)
                    ...
                    <div class="mt-4 flex flex-wrap justify-center gap-2">
                        <span
                            class="px-3 py-1 text-xs font-semibold rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30">
                            {{ $product->category->name ?? 'Tanpa Kategori' }}
                        </span>
                        <span
                            class="px-3 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400 border border-blue-200 dark:border-blue-500/30">
                            {{ $product->units->where('is_base_unit', true)->first()->unit->name ?? 'Pcs' }}
                        </span>
                    </div>
                </div>
                ...
                <div class="card p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div
                            class="p-3 rounded-xl bg-gray-50 dark:bg-gray-700/30 border border-gray-100 dark:border-gray-700">
                            <span class="block text-xs text-gray-500 dark:text-gray-400 mb-1">Total Stok</span>
                            <span
                                class="block text-xl font-bold text-gray-800 dark:text-gray-100">{{ $product->stock }}</span>
                        </div>
                        <div
                            class="p-3 rounded-xl bg-gray-50 dark:bg-gray-700/30 border border-gray-100 dark:border-gray-700">
                            <span class="block text-xs text-gray-500 dark:text-gray-400 mb-1">Min. Stok</span>
                            <span
                                class="block text-xl font-bold text-gray-800 dark:text-gray-100">{{ $product->min_stock }}</span>
                        </div>
                    </div>
                </div>
            </div>
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                            @if ($batch->expire_date)
                                                <span
                                                    class="{{ $batch->expire_date < now() ? 'text-red-500 font-bold' : '' }}">
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                @forelse ($product->stockMovements()->latest()->take(10)->get() as $movement)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                        ...
                                        <td
                                            class="px-6 py-4 text-sm font-mono font-semibold {{ $movement->type == 'in' ? 'text-emerald-600' : 'text-red-500' }}">
                                            {{ $movement->type == 'in' ? '+' : '-' }}{{ abs($movement->quantity) }}
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                            {{ $movement->reference_type }} #{{ $movement->reference_id }}
                                            @if ($movement->description)
                                                <div class="text-xs text-gray-400 mt-0.5">{{ $movement->description }}
                                                </div>
                                            @endif
                                        </td>
                                    </tr>
