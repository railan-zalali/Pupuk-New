<x-app-layout>
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8" x-data="{
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-400 dark:text-gray-500 mt-0.5">
                            Perbarui informasi transaksi penjualan <span class="font-semibold text-gray-600 dark:text-gray-300">{{ $sale->invoice_number }}</span>
                        </p>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    @if ($sale)
                    <a href="{{ route('sales.drafts') }}" class="btn-ghost btn-sm">
                        <i class="ti ti-arrow-left text-base"></i> Kembali ke Draft
                    </a>
                    @else
                    <a href="{{ route('sales.index') }}" class="btn-ghost btn-sm">
        ...
        @if ($sale)
        <div class="mb-5 flex items-center gap-3 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
            <i class="ti ti-info-circle text-amber-500 text-lg flex-shrink-0"></i>
            <p class="text-sm text-amber-700 dark:text-amber-300">
                Memproses draft penjualan. Tekan <strong>Simpan Transaksi</strong> untuk menyelesaikan atau <strong>Simpan Sebagai Draft</strong> untuk menyimpan.
            </p>
        </div>
        @endif

        @if ($errors->any())
        <div class="flex items-start gap-3 p-4 mb-5 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20">
            <i class="ti ti-alert-circle text-red-500 text-lg flex-shrink-0 mt-0.5"></i>
            <div>
                <h3 class="text-sm font-semibold text-red-800 dark:text-red-200">Ada kesalahan:</h3>
        ...
        <form action="{{ route('sales.update', $sale->id) }}" method="POST" id="saleForm" class="space-y-6">
            @csrf
            @method('PUT')

            <!-- Card: Informasi Penjualan -->
            <div class="card overflow-hidden">
                <div class="flex items-center gap-2 p-5 border-b border-gray-100 dark:border-gray-700/40">
                    <i class="ti ti-file-invoice text-emerald-500"></i>
                    <div>
                        <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-100">Informasi Penjualan</h3>
                        <p class="text-xs text-gray-400 mt-0.5">Detail transaksi dan informasi pelanggan</p>
                    </div>
                </div>
                            ...
                            <div class="mt-1">
                                <select id="customer_select" name="customer_id"
                                    ...
                                    <option value="{{ $customer->id }}" {{ old('customer_id', $sale->customer_id) == $customer->id ? 'selected' : '' }}>
                                        {{ $customer->nama }} - {{ $customer->kecamatan_nama }}, {{ $customer->kabupaten_nama }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-span-1">
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <select id="payment_method" name="payment_method"
                                    ...
                                    <option value="credit" {{ old('payment_method', $sale->payment_method) == 'credit' ? 'selected' : '' }}>Kredit</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-span-1">
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <select id="vehicle_type" name="vehicle_type"
                                    class="pl-10 mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:text-gray-300">
                                    <option value="Truk" {{ old('vehicle_type', $sale->vehicle_type) == 'Truk' ? 'selected' : '' }}>Truk</option>
                                    <option value="Pickup" {{ old('vehicle_type', $sale->vehicle_type) == 'Pickup' ? 'selected' : '' }}>Pickup</option>
                                </select>
                            </div>
                        </div>
            ...
            <div class="card overflow-hidden">
                <div class="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-700/40">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-100">Item Penjualan</h3>
                            <p class="text-xs text-gray-400 mt-0.5">Tambahkan produk yang dibeli</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        <button type="button" onclick="openProductSelector()" class="btn-primary btn-sm">
                            <i class="ti ti-package text-base"></i> Pilih Produk
                        </button>
                        <button type="button" onclick="addItem()" class="btn-ghost btn-sm">
            ...
            <div class="card overflow-hidden">
                <div class="flex items-center gap-2 p-5 border-b border-gray-100 dark:border-gray-700/40">
                    <i class="ti ti-cash text-emerald-500"></i>
                    <div>
                        <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-100">Detail Pembayaran</h3>
                        <p class="text-xs text-gray-400 mt-0.5">Informasi pembayaran dan diskon</p>
                    </div>
                </div>
                                    ...
                                    <div class="flex justify-between items-center">
                                        ...
                                        <div class="relative w-1/2">
                                            ...
                                            <input type="number" name="discount" id="discount"
                                                ...
                                                value="{{ old('discount', $sale->discount) }}" min="0" onchange="calculateFinalTotal()"
                                                placeholder="0">
                                        </div>
                                    </div>
                                        ...
                                        <div class="flex space-x-2">
                                            <div class="relative">
                                                ...
                                                <input type="number" name="down_payment" id="down_payment"
                                                    ...
                                                    value="{{ old('down_payment', $sale->payment_method == 'credit' ? $sale->paid_amount : 0) }}" min="0"
                                        ...
                                        <div class="flex space-x-2">
                                            <div class="relative">
                                                ...
                                                <input type="number" name="paid_amount" id="paid_amount"
                                                    class="pl-10 w-32 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 transition-colors"
                                                    value="{{ old('paid_amount', $sale->paid_amount) }}" min="0" onchange="calculateChange()">
                                            </div>
            ...
            <div class="card overflow-hidden">
                <div class="flex items-center gap-2 p-5 border-b border-gray-100 dark:border-gray-700/40">
                    <i class="ti ti-notes text-emerald-500"></i>
                    <div>
                        <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-100">Catatan</h3>
                        <p class="text-xs text-gray-400 mt-0.5">Informasi tambahan untuk transaksi ini</p>
                    </div>
                </div>

                <div class="px-4 py-5 sm:p-6">
                    <textarea id="notes" name="notes" rows="3"
                        class="shadow-sm block w-full focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 rounded-xl transition-colors"
            ...
            <div class="flex justify-end gap-3 py-4">
                ...
                <div class="relative group">
                    <button type="submit" name="save_draft" value="1"
                        class="inline-flex items-center gap-1.5 px-4 py-2.5 border border-amber-400 dark:border-amber-500/40 text-sm font-medium rounded-xl text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-500/10 hover:bg-amber-100 dark:hover:bg-amber-500/20 focus:outline-none focus:ring-2 focus:ring-amber-500/40 transition-all">
                        <i class="ti ti-file-text text-base"></i>
                        @if ($sale) Perbarui Draft @else Simpan sebagai Draft @endif
                    </button>
                </div>
    ...
    <div id="shortcuts-modal" class="fixed inset-0 overflow-y-auto hidden z-50" aria-labelledby="modal-title"
        ...
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <!-- Modal Header -->
                <div class="bg-emerald-600 dark:bg-emerald-700 px-5 py-4 flex justify-between items-center">
                    ...
                    <button type="button" id="close-shortcuts-modal"
                        class="rounded-xl bg-white/10 hover:bg-white/20 p-1.5 text-white transition-colors">
    ...
    <script>
        ...
        function addAutoSaveIndicator() {
            const indicator = `
                    <div id="auto-save-indicator" class="fixed top-20 right-6 bg-white dark:bg-gray-800 px-4 py-2 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 hidden">
                    ...
                    <div id="auto-save-success" class="fixed top-20 right-6 bg-green-50 dark:bg-green-900/50 px-4 py-2 rounded-lg shadow-lg border border-green-200 dark:border-green-800 hidden">
                        <div class="flex items-center space-x-2">
                            ...
                            <span class="text-sm text-green-600 dark:text-green-300">Draft tersimpan otomatis <span id="save-time"></span></span>
                        </div>
                    </div>
                `;
        ...
        function watchForChanges() {
            ...
            $('#saleForm').on('change input', 'input, select, textarea', function() {
                isDraftSaved = false;
            });
        }

        function shouldAutoSave() {
            // Check if there are items and form has been modified
            const hasItems = $('#saleItems tr').length > 0;
            const hasProductSelected = $('#saleItems select[name="product_id[]"]').filter(function() {
                return $(this).val();
            }).length > 0;

            return hasItems && hasProductSelected && !isDraftSaved;
        }

        function performAutoSave() {
            // Show saving indicator
            $('#auto-save-indicator').removeClass('hidden');
        ...
        function loadDraftData() {
            ...
            $('#payment_method').val('{{ $sale->payment_method }}').trigger('change');
            @endif
            ...
            try {
                ...
                if (productSelect && productSelect.length > 0) {
                    if (productSelect.find(`option[value="{{ $detail->product_id }}"]`).length === 0) {
                        ...
                        @if($detail - > product)
                        productSelect.append(new Option('{{ $detail->product->name }}', {
                            {
                                $detail - > product_id
                            }
                        }, true, true));
                ...
                setTimeout(() => {
                    try {
                        ...
                        // Log untuk debugging
                        console.log('Setting unit, quantity, and price for product:', {
                            {
                                $detail - > product_id
                            }
                        });
                        ...
                        if (unitSelect && unitSelect.length > 0) {
                            ...
                            if (unitSelect.find(`option[value="{{ $detail->unit_id }}"]`).length ===
                                0) {
                                @if($detail - > unit)
                                console.log(
                                    'Adding missing unit option: {{ $detail->unit->name }}');
                                unitSelect.append(new Option('{{ $detail->unit->name }}', {
                                    {
                                        $detail - > unit_id
                                    }
                                }, true, true));
                            ...
                            setTimeout(() => {
                                ...
                                const quantityInput = currentRow.find(
                                    'input[name="quantity[]"]');
            ...
            setTimeout(() => {
                try {
                    ...
                    showNotification('Data penjualan berhasil dimuat', 'success');
                } catch (error) {
