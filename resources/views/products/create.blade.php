                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input type="number" name="expiry_warning_days" id="expiry_warning_days" 
                                    min="0" step="1" value="30" class="mt-1 block w-full" />
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                ...
                                <div class="space-y-4">
                                    <div class="flex items-center">
                                        <input type="checkbox" name="requires_expiry_date" id="requires_expiry_date" 
                                            value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700">
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700"
                                    ...
                                    <tr class="product-row">
                                        ...
                                        <td class="px-3 py-3 whitespace-nowrap">
                                            <input type="checkbox" name="products[0][is_perishable]" value="1"
                                                class="is-perishable-checkbox rounded border-gray-300 dark:border-gray-600 text-blue-600 shadow-sm focus:border-blue-500 dark:focus:border-blue-500 focus:ring-blue-500 dark:focus:ring-blue-500">
                                        </td>
                                        <td class="px-3 py-3 whitespace-nowrap">
                                            <input type="date" name="products[0][expire_date]"
                                                class="expire-date-input block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 dark:focus:border-blue-500 focus:ring-blue-500 dark:focus:ring-blue-500 text-sm"
                                                disabled>
                                            <div class="expiry-validation-message text-red-500 text-xs mt-1 hidden"></div>
                                        </td>
                                        <td class="px-3 py-3 whitespace-nowrap">
                                            <input type="number" name="products[0][expiry_warning_days]" placeholder="7"
                                                min="1" max="365"
                                                class="expiry-warning-input block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 dark:focus:border-blue-500 focus:ring-blue-500 dark:focus:ring-blue-500 text-sm"
                                                disabled>
                                        </td>
                                        <td class="px-3 py-3 whitespace-nowrap">
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (addProductRowBtn && productRowsContainer) {
                    ...
                    addProductRowBtn.addEventListener('click', function() {
                        ...
                        newRow.innerHTML = `
                            ...
                            <td class="px-3 py-3 whitespace-nowrap">
                                <input type="checkbox" name="products[${productRowCounter}][is_perishable]" value="1"
                                    class="is-perishable-checkbox rounded border-gray-300 dark:border-gray-600 text-blue-600 shadow-sm focus:border-blue-500 dark:focus:border-blue-500 focus:ring-blue-500 dark:focus:ring-blue-500">
                            </td>
                            <td class="px-3 py-3 whitespace-nowrap">
                                <input type="date" name="products[${productRowCounter}][expire_date]"
                                    class="expire-date-input block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 dark:focus:border-blue-500 focus:ring-blue-500 dark:focus:ring-blue-500 text-sm"
                                    disabled>
                                <div class="expiry-validation-message text-red-500 text-xs mt-1 hidden"></div>
                            </td>
                            <td class="px-3 py-3 whitespace-nowrap">
                                <input type="number" name="products[${productRowCounter}][expiry_warning_days]" placeholder="7"
                                    min="1" max="365"
                                    class="expiry-warning-input block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 dark:focus:border-blue-500 focus:ring-blue-500 dark:focus:ring-blue-500 text-sm"
                                    disabled>
                            </td>
                ...
                // Batch form expiry validation
                function setupBatchExpiryValidation() {
                    function toggleBatchExpiryValidation(row) {
                        const isPerishableCheckbox = row.querySelector('.is-perishable-checkbox');
                        const expireDateInput = row.querySelector('.expire-date-input');
                        const expiryWarningInput = row.querySelector('.expiry-warning-input');
                        const validationMessage = row.querySelector('.expiry-validation-message');

                        if (!isPerishableCheckbox || !expireDateInput || !expiryWarningInput) return;

                        const isPerishable = isPerishableCheckbox.checked;

                        // Toggle required attributes and disabled state
                        expireDateInput.required = isPerishable;
                        expireDateInput.disabled = !isPerishable;
                        expiryWarningInput.required = isPerishable;
                        expiryWarningInput.disabled = !isPerishable;

                        // Set default values and minimum date
                        if (isPerishable) {
                            const today = new Date();
                            const tomorrow = new Date(today);
                            tomorrow.setDate(tomorrow.getDate() + 1);
                            expireDateInput.min = tomorrow.toISOString().split('T')[0];
                            
                            if (!expireDateInput.value) {
                                const defaultDate = new Date(today);
                                defaultDate.setDate(defaultDate.getDate() + 30);
                                expireDateInput.value = defaultDate.toISOString().split('T')[0];
                            }
                            
                            if (!expiryWarningInput.value) {
                                expiryWarningInput.value = '7';
                            }
                        } else {
                            expireDateInput.value = '';
                            expiryWarningInput.value = '';
                            expireDateInput.removeAttribute('min');
                        }

                        // Show/hide validation message
                        if (validationMessage) {
                            validationMessage.style.display = isPerishable ? 'block' : 'none';
                            validationMessage.textContent = isPerishable ? 
                                'Tanggal kedaluwarsa dan hari peringatan wajib diisi untuk produk yang mudah rusak.' : '';
                        }
                    }

                    // Setup validation for existing rows
                    document.querySelectorAll('.product-row').forEach(row => {
                        const isPerishableCheckbox = row.querySelector('.is-perishable-checkbox');
                        if (isPerishableCheckbox) {
                            isPerishableCheckbox.addEventListener('change', () => toggleBatchExpiryValidation(row));
                            toggleBatchExpiryValidation(row); // Initial setup
                        }
                    });

                    // Setup validation for new rows (using event delegation)
                    document.addEventListener('change', function(e) {
                        if (e.target.classList.contains('is-perishable-checkbox')) {
                            const row = e.target.closest('.product-row');
                            if (row) {
                                toggleBatchExpiryValidation(row);
                            }
                        }
                    });
                }

                // Initialize batch expiry validation
                setupBatchExpiryValidation();

                // Re-setup validation when new rows are added
                const originalAddEventListener = addProductRowBtn.addEventListener;
                if (addProductRowBtn) {
                    addProductRowBtn.addEventListener('click', function() {
                        setTimeout(() => {
                            const newRow = document.querySelector('.product-row:last-child');
                            if (newRow) {
                                const isPerishableCheckbox = newRow.querySelector('.is-perishable-checkbox');
                                if (isPerishableCheckbox) {
                                    isPerishableCheckbox.addEventListener('change', () => toggleBatchExpiryValidation(newRow));
                                    toggleBatchExpiryValidation(newRow); // Initial setup
                                }
                            }
                        }, 100);
                    });
